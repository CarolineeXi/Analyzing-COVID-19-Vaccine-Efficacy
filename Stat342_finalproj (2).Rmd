...
Results: 
In both cases, the p-value approached 0, indicating sufficient evidence to reject the null hypothesis $\psi_0 = 0.3$. Efficacy rates were calculated to be between 91.48% and 98.65%, indicating strong evidence supporting the reduction in symptomatic COVID-19.
...
ggplot(data, aes(x = group, y = num_cases)) + 
    geom_col(fill = "#0099f9", width = 0.6) +  labs(
      title = "Number of Covid-19 Cases in Group Receiving BNT162b2 Vaccine vs. Placebo", y= "Number of cases at least 7 days after receiving dose"
...
Hence, we get the P-value under the null hypothesis is $2.822294 \times 10^{-28}$ and the empirical P-value by simulation is 0, confirming the significance with an empirical p-value is also less than 0.001.
...
$$[0.9148, 0.9865]$$
Besides, the empirical P-value by bootstrap is also 0.
...
ggplot(data = sample_summary) +
  ...
  labs(x = "Sample Confidence Interval", y = "Bootstrap Samples",
       title = "95% Confidence Interval for 100 Bootstrap Samples") +
  theme_minimal() +
...
Figure 3. 95% Confidence Interval for 100 Bootstrap Samples.

The plot shows the 95% confidence interval for the vaccine efficacy estimate, with the observed MOM estimate indicated by a red dashed line and a blue point. This visualization helps to see the variability and central tendency of the MOM estimates obtained from bootstrapping
...
Theorem 25.1 can be used under certain regularity conditions. When n is large, and the regularity conditions hold, the log-likelihood function can be well approximated by a second order Taylor series around the MLE. 
```{r,warning=FALSE}
