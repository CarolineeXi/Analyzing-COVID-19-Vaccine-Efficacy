...
Our maximum likelihood estimate of the vaccine efficcy $\psi$ was $\frac{n-2x}{n-x} = \frac{170-2\cdot8}{170-8} = 0.9506$. As the likelihood ratio test gave an extremely low p-value of $2.822294 \times 10^{-28}$, we can confidently reject the null hypothesis that $\psi = 0.3$. Under the method of empirical p-values, we calculated a p-value that is extremely close to $0$, and in the plot we can see that our calculated value of $W$ is very far away from any of the values of $W$ under the null distribution. However, one weakness of using the maximum likelihood estimate is that the regularity condition does not hold, as the plot of the log-likelihood differs substantially from the Taylor approximation. Still, the functions are quite similar around the value of the MLE, which indicates that this method can still be an acceptable way of estimating the efficacy rate.
