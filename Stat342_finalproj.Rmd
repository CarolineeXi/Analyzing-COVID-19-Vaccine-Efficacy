...
$l"(\phi_0^{mle}) = -\frac{x}{(1-\phi_0)^2} + \frac{n}{(2-\phi_0)^2} = \frac{-x(2-\phi_0)^2+n(1-\phi_0)^2}{(1-\phi_0)^2(2-\phi_0)^2}$

Now let check the second derivative to see if $\phi_0^{mle}$ is maximum likelihood estimate of $\phi_0$ 

let's see if $l"(\phi_0^{mle}) < 0$
$$
\begin{align*}
l"(\phi_0^{mle}) &<0 \\
\frac{-x}{(1-\phi_0)^2} + \frac{n}{(2-\phi_0)^2} &< 0 \\
\frac{n}{(2-\phi_0)^2} &< \frac{x}{(1-\phi_0)^2}\\
=> (1-\phi_0)^2 &< (2-\phi_0)^2 \\
|1-\phi_0| &< |2-\phi_0| \\
\end{align*}
$$
As given the condition $-\infty < \phi_0 \leq 1$ and knowing $x<n$, 

we see that $(1-\phi_0)^2 > (2-\phi_0)^2$ if and only if $0 < \phi_0 \leq 1$; 

while $\phi_0 \leq 0$ will give the minimum, which is not possible because it does not satisfy the derived inequality for finding maximum and is not logical in the context of the vaccine efficacy, indicating a non-existent $\phi_0^{mle}$. 
...
$$
\begin{align*}
SE(\phi_0^{mle}) &= \frac{(1-\phi_0^{mle})(2-\phi_0^{mle})}{\sqrt{-x(2-\phi_0^{mle})^2+n(1-\phi_0^{mle})^2}} \\
\end{align*}
$$
```{r}
se <- 170*(1 - mle)^2 - 8*(2-mle)^2
se
```
...
$$
\begin{align*}
l(\phi_0^{mle}) &= ln\binom{n}{x} +ln(1-\phi)^x - ln(2-\phi)^n
\end{align*}
$$
